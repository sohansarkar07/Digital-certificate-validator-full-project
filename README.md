...
| 🔐 **Soroban Contract — Major Expansion** | ✅ Done | Completely rewrote the core `CertificateValidator` Soroban contract with **13 new functions**: `issue_typed_credential` (full metadata with issuer wallet auth), `batch_verify` (verify multiple hashes in one call), `suspend_certificate` / `restore_certificate`, `is_expired`, `get_credential`, `get_all_hashes`, `transfer_admin`, `remove_sub_admin`, `get_ledger_timestamp`; added `CredentialStatus` enum and `ipfs_doc_hash` field. All **13 unit tests pass** locally (`cargo test`). | [2123221](https://github.com/sohansarkar07/Digital-certificate-validator-full-project/commit/2123221) |
