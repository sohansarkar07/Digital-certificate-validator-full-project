...
import { CheckCircle, XCircle, FileImage, ShieldCheck, Database, SearchCode, LockKeyhole, Copy, Check, ExternalLink, Loader2 } from "lucide-react";
  ...
  const verifyFile = useCallback(async (file: File) => {
    ...
    setTimeout(async () => {
        try {
          ...
          const isValid = await contractService.verifyCertificate(hashHex, address || undefined, sign);
                            ...
                            <div className="p-4 bg-surface text-sm space-y-4">
                                ...
                                {status === 'valid' && hash && (
                                    <motion.div
                                      ...
                                      className="mt-4 flex flex-col items-center gap-3 p-4 bg-secondary rounded border border-border"
                                    >
                                      <span className="text-[10px] font-bold text-foreground/40 uppercase tracking-widest self-start">Shareable QR Code</span>
                                      <QRCodeDisplay
                                        value={`${typeof window !== 'undefined' ? window.location.origin : ''}/verify/${hash}`}
                                        size={120}
                                        label="Scan to Verify"
                                      />
                                      <p className="text-[10px] text-foreground/40 text-center leading-relaxed">
                                        Scan to instantly verify this certificate on any device.
                                      </p>
                                    </motion.div>
                                )}
